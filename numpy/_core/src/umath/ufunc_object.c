        ...
        npy_bool outer)
{
    ...
    /*
     * Happy path: nout==1, no outer, no output args, no kwargs,
     * all inputs are exact ndarrays, not a generalized ufunc.
     * Skips override checking, argument parsing, and wrapping overhead.
     */
    int happy_path = (nout == 1) && !outer && !out_is_passed_by_position
            && (kwnames == NULL) && !ufunc->core_enabled;
    for (int i = 0; i < nin && happy_path; i++) {
        happy_path = PyArray_CheckExact(args[i]);
    }

    if (happy_path) {
            /*
             * Inline convert_ufunc_arguments: we know all inputs are exact
             * arrays, no outputs, no kwargs to process.
             * Note: operands need strong refs because GenericFunctionInternal
             * may replace them with cast copies (DECREFing the original).
             */
            for (int i = 0; i < nin; i++) {
                operands[i] = (PyArrayObject *)Py_NewRef(args[i]);
                operand_DTypes[i] = NPY_DTYPE(PyArray_DESCR(operands[i]));
            ...
            if (resolve_descriptors(nop, ufunc, ufuncimpl,
                    ...
                    ufunc_input, NPY_DEFAULT_ASSIGN_CASTING) < 0) {
                goto fail;
            }

            errval = PyUFunc_GenericFunctionInternal(ufunc, ufuncimpl,
                    operation_descrs, operands, NPY_DEFAULT_ASSIGN_CASTING,
                    NPY_KEEPORDER, NULL);
            if (errval < 0) {
                goto fail;
            }

            /* Clean up (from here on, we cannot `goto fail`) */
            for (int i = 0; i < nop; i++) {
            ...
            PyObject *result = (PyObject *)(operands[nin]);
            npy_free_workspace(scratch_objs);

            /* Convert 0-d array result to scalar */
            if (return_scalar && PyArray_NDIM((PyArrayObject *)result) == 0) {
                            ...
                            void *data)
{
    ...
    for (Py_ssize_t j = 0; j < PyList_GET_SIZE(ufunc->_loops); j++) {
        ...
        PyArrayMethodObject *reg_meth = (PyArrayMethodObject *)registered;
        if (!PyObject_TypeCheck(registered, &PyArrayMethod_Type)
                || (reg_meth->get_strided_loop != &get_wrapped_legacy_ufunc_loop
