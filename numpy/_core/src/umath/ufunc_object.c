...
/*
 * Fast path for a plain unary call on a trivially iterable exact array.
        ...
        PyArray_DTypeMeta *in_DType, PyObject **result)
{
    ...
    else if (!PyArray_IS_C_CONTIGUOUS(in)) {
    ...
    PyArrayObject *out = (PyArrayObject *)PyArray_NewFromDescr(
            ...
            PyArray_NDIM(in), PyArray_SHAPE(in), NULL, NULL, out_fortran,
            NULL);
    if (out == NULL) {
        return -1;
    }
        ...
        npy_bool outer)
{
    ...
    if (nin == 1 && nout == 1 && !ufunc->core_enabled
        ...
        int fast_status = try_unary_trivial_call(
